...
func (dec *Decoder) ExpectMailbox(ptr *string) bool {
	...
	if err != nil {
		// A name that is not modified UTF-7 is a malformed command, not a
		// server fault: the connection loop answers BAD for an expect error
		// and NO [SERVERBUG] for anything else.
...
func (dec *Decoder) ExpectNumSet(kind NumKind, ptr *imap.NumSet) bool {
	...
	if err != nil {
		// Same as above: the client sent something the grammar does not allow.
		return dec.returnErr(&DecoderExpectError{
